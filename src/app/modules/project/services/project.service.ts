...
export class ProjectService {
  ...
  patchFromForm(project: Project, id: number): Observable<Project> {
    const projectToPatch = {
      ...project,
    };
    return this.httpClient
      .patch<Project>(`http://localhost:3000/projects/${id}`, projectToPatch)
      .pipe(
        tap((data) => this.logService.log(`patch used ${data}`)),
        retry(3),
        catchError(this.handleError)
      );
  }
  ...
  erase(id: number): Observable<Project> {
    return this.httpClient
      .delete<Project>(`http://localhost:3000/projects/${id}`)
      .pipe(
